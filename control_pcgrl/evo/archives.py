...
class CMAInitStatesGrid(InitStatesArchive):
    ...
    def add(self, solution, objective_value, behavior_values, meta, index=None):
        status, dtype_improvement = GridArchive.add(
            self, solution, objective_value, behavior_values
        )
        ...
        if status != AddStatus.NOT_ADDED:
            ...
            archive_init_states(
                self.init_states_archive,
                self.door_coords_archive,
                self.init_states,
                self.door_coords,
                index,
            )
...
class MEInitStatesArchive(InitStatesArchive, MEGrid):
    ...
    def __init__(self, bin_sizes, bin_bounds, n_init_states, map_dims, **kwargs):
        InitStatesArchive.__init__(
            self, bin_sizes, bin_bounds, n_init_states, map_dims, **kwargs
        )
...
@njit
def archive_init_states(
    init_states_archive, door_coord_archive, init_states, door_coords, index
):
