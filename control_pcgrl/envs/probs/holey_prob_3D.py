...
class HoleyProblem3D(HoleyProblem, Problem3D):
    ...
    def get_border_idxs(self):
        dummy_bordered_map = np.zeros(
            (self._height + 2, self._width + 2, self._length + 2), dtype=np.uint8
        )
    ...
    def gen_all_holes(self):
        ...
        hole_pairs = [
            ...
            if self._valid_holes((pair[0], pair[0] + np.array([1, 0, 0])), pair[1])
        ]
        hole_pairs = [
            (
                (pair[0], pair[0] + np.array([1, 0, 0])),
                (pair[1], pair[1] + np.array([1, 0, 0])),
            )
            for pair in hole_pairs
        ]
        return hole_pairs
    ...
    def gen_holes(self):
        ...
        if len(self._hole_queue) > 0:
            (self.entrance_coords, self.exit_coords), self._hole_queue = (
                self._hole_queue[0],
                self._hole_queue[1:],
            )
            ...
            self.entrance_coords = np.array(
                ((1, 0, self._length), (2, 0, self._length))
            )
            self.exit_coords = np.array(
                ((2, self._width + 1, 1), (3, self._width + 1, 1))
            )
            ...
            idxs = np.random.choice(
                self._border_idxs.shape[0], size=potential, replace=False
            )
    ...
    def _valid_holes(self, entrance_coords, exit_coords) -> bool:
        ...
        return (
            np.max(
                (
                    np.abs(entrance_coords[0] - exit_coords),
                    np.abs(entrance_coords[1] - exit_coords),
                )
            )
            > 1
        )
