...
BBOX = (
    BBOX_OFFSET[0],
    BBOX_OFFSET[1],
    BBOX_OFFSET[0] + SCREENSHOT_SIZE[0],
    BBOX_OFFSET[1] + SCREENSHOT_SIZE[1],
)
...
def spawn_base(
    map, border_size=(1, 1, 1), base_pos=5, boundary_size=3, backgroud_type=QUARTZ_BLOCK
):
    ...
    if entrance_coords is not None:
        CLIENT.fillCube(
            FillCubeRequest(
                cube=Cube(
                    min=Point(
                        x=entrance_coords[0][2],
                        y=entrance_coords[0][0],
                        z=entrance_coords[0][1],
                    ),
                    max=Point(
                        x=entrance_coords[1][2],
                        y=entrance_coords[1][0],
                        z=entrance_coords[1][1],
                    ),
                ),
                type=AIR,
            )
            ...
            for i in range(len(map[k][j])):
                ...
                blocks.append(
                    Block(
                        position=Point(
                            x=i + 1 + offset[0],
                            y=k + base_pos + 1 + offset[2],
                            z=j + 1 + offset[1],
                        ),  # NOTE: the -1 may cause a problem when the border is thicker than 1
                        type=item,
                        orientation=NORTH,
                    )
                )
        ...
        CLIENT.spawnBlocks(
            Blocks(
                blocks=[
                    Block(
                        position=Point(
                            x=points[0][0] + offset[0],
                            y=points[0][2] + base_pos + offset[2],
                            z=points[0][1] + offset[1],
                        ),
                        type=item,
                    ),
                    Block(
                        position=Point(
                            x=points[0][0] + offset[0],
                            y=points[0][2] + base_pos + offset[2] + 1,
                            z=points[0][1] + offset[1],
                        ),
                        type=AIR,
                    ),
    ...
    CLIENT.spawnBlocks(
        Blocks(
            blocks=[
                Block(
                    position=Point(
                        x=entrance[2], y=entrance[0] + base_pos - 1, z=entrance[1]
                    ),
                    type=GOLD_BLOCK,
                    orientation=NORTH,
                ),
                Block(
                    position=Point(x=exit[2], y=exit[0] + base_pos - 1, z=exit[1]),
                    type=DIAMOND_BLOCK,
                    orientation=NORTH,
                ),
            ]
...
def top_left_corner_screenshot(name: str, bbox: tuple, save_dir: str):
    ...
    if np.array(im).shape != (512, 512, 4):
        raise Exception(
            f"Screenshot is not 512x512, size is {np.array(im).shape}. Has the display gone to sleep?"
        )
...
if __name__ == "__main__":
    ...
    CLIENT.fillCube(
        FillCubeRequest(
            cube=Cube(min=Point(x=-50, y=3, z=-50), max=Point(x=50, y=3, z=50)),
            type=GRASS,
        )
    )
    CLIENT.fillCube(
        FillCubeRequest(
            cube=Cube(min=Point(x=-50, y=4, z=-50), max=Point(x=50, y=15, z=50)),
            type=AIR,
        )
    )
