...
def _passable(map, x, y, z, n_j, passable_values):
    ...
    for dir in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
        ...
        # need to garantee there is extra head room above the foothold.
            ...
            < len(
                map
            )  # Our head must remain inside the map and the empty space below must >= 2
            and map[nz + 2][ny][nx]
            in passable_values  # five blocks ahead must be passable
            and map[nz + 1][ny][nx] in passable_values
            and map[nz][ny][nx] in passable_values
            and map[nz - 1][ny][nx] in passable_values
            and map[nz - 2][ny][nx] in passable_values
            and map[nz + 2][y][x]
            in passable_values  # The extra 1 head-room at the starting point must be passable
            and jx >= 0
            and jy >= 0
            and jx < len(map[z][y])
            and jy < len(map[z])  # The foothold must be in the map
        ):
            if (  # the height difference is 0
                map[jz + 1][jy][jx] in passable_values  # head room at the foothold
                and map[jz + 2][jy][jx]
                in passable_values  # extra head room at the foothold
                ...
                passable_tiles[(jx, jy, jz + 1, n_j + 1)] = [
                    (nx, ny, nz),
                    (nx, ny, nz + 1),
                ]
...
def run_dijkstra(x, y, z, map, passable_values):
    ...
    while len(queue) > 0:
        ...
        for foothold, traversed in _passable(
            map, cx, cy, cz, jumps[(cx, cy, cz)], passable_values
        ).items():
...
def get_path_coords(path_map, x=None, y=None, z=None, can_fly=False):
    ...
    while max_cell > 1:
        ...
        adj_mask = np.zeros(
            shape=(
                max(height, ADJ_FILTER.shape[0]) + 2,
                max(width, ADJ_FILTER.shape[1]) + 2,
                max(length, ADJ_FILTER.shape[2]) + 2,
            ),
            dtype=np.int32,
        )
...
def gen_random_map(random, dims, prob):
    map = random.choice(list(prob.keys()), size=dims, p=list(prob.values())).astype(
        np.uint8
    )
    return map
